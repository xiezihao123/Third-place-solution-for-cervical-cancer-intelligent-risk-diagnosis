...
def results_postprocess():
  ...
  for file in files:
    if os.path.isdir(path+file):
      ...
      os.remove(path+file)
  pkl_file = open('/home/admin/jupyter/mmdetection-master/result/res_hrnetw32_epoch7.pkl', 'rb')
  data = pickle.load(pkl_file)
  pkl_file2 = open('/home/admin/jupyter/mmdetection-master/result/res_hrnetw64_epoch5.pkl', 'rb')
  data2 = pickle.load(pkl_file2)
  for i in range(len(data)):
    for j in range(6):
      data[i][j] = np.concatenate((data[i][j], data2[i][j]),0)
...
def duplicate_removal(result):
  testroi = []
  f = open('/home/admin/jupyter/tianchi_data/test/VOC2007_roi/ImageSets/Main/test.txt', 'r')
  for line in f.readlines():
    testroi.append(line.split('\n')[0])
  f.close()
  pkl_file = open('/home/admin/jupyter/mmdetection-master/result/res_roi.pkl', 'rb')
  data = pickle.load(pkl_file)
  nms_thres = [0.4,0.4,0.4,0.4,0.4,0.4]
  
  for key in result.keys():
    if key.find('6198')!=-1 or key.find('9379')!=-1:
      continue
    print('index:',testroi.index(key))
    roi_dt = data[testroi.index(key)]    
    for k in range(len(roi_dt)):
        roi_dt[k] = roi_dt[k][roi_dt[k][:,4]>0.1]
        roi_dt[k] = roi_dt[k][((roi_dt[k][:,3]-roi_dt[k][:,1])*(roi_dt[k][:,2]-roi_dt[k][:,0]))>90000]
        #roi_dt[k] = filter_boxinbox(roi_dt[k])
    ...
    for i in range(6):
        ...
        roi_dets[i] = compare_area(roi_dets[i], roi_dt[i])
        roi_dets[i] = np.concatenate((roi_dets[i], roi_dt[i]),0)
#         if i == 4:
#             roi_det_tmp = filter_boxinbox(roi_dets[i][roi_dets[i][:,4]>0.1])
#             roi_dets[i] = np.concatenate((roi_dets[i][roi_dets[i][:,4]<=0.1], roi_det_tmp),0)

    f = open('/home/admin/jupyter/tianchi_data/test/ROI_coord/'+key+'.txt', 'r')
    lines = f.readlines()
    f.close()
    coord = lines[0].split()
    for k in range(6):
      roi_dets[k] += np.array([float(coord[0]), float(coord[1]), float(coord[0]), float(coord[1]), 0])
    
    roi_dets = filter_noise(roi_dets)
    res = []
    for i in range(6):
    ...
    save_dir = './tianchi_json/'+key+'.json'
    f =open(save_dir, 'w')
    json.dump(res ,f)
    f.close()
...
def filter_noise(data):
    sum_ah = data[0][data[0][:,4]>0.05][:,4].sum()
    sum_as = data[1][data[1][:,4]>0.05][:,4].sum()
    sum_hl = data[2][data[2][:,4]>0.05][:,4].sum()
    sum_ll = data[3][data[3][:,4]>0.05][:,4].sum()
    sum_ca = data[4][data[4][:,4]>0.05][:,4].sum()
    sum_th = data[5][data[5][:,4]>0.05][:,4].sum()
    sum_aa = sum_ah+sum_as+sum_hl+sum_ll
    
#     if sum_aa>2*(sum_ca+sum_th) and sum_aa>5:
#         return [data[0], data[1], data[2], data[3], np.array([]).reshape(0,5),np.array([]).reshape(0,5)]
    
#     if sum_ca>2*(sum_aa+sum_th) and sum_ca>5:
#         return [np.array([]).reshape(0,5),np.array([]).reshape(0,5), np.array([]).reshape(0,5),np.array([]).reshape(0,5), data[4], np.array([]).reshape(0,5)]
    
#     if sum_th>2*(sum_aa+sum_ca) and sum_th>5:
#         return [np.array([]).reshape(0,5),np.array([]).reshape(0,5), np.array([]).reshape(0,5),np.array([]).reshape(0,5), np.array([]).reshape(0,5) ,data[5]]
    if (sum_aa>sum_th and sum_aa>5) or (sum_ca>sum_th and sum_ca>5):
        return [data[0], data[1], data[2], data[3], data[4],np.array([]).reshape(0,5)]
    return data
...
def compare_area(a, b, ratio=1, thres=0.5):
  if b.shape[0]==0 or b.shape[1]==0 or a.shape[0]==0 or a.shape[1]==0:
    return a
  boxes_a = torch.from_numpy(a[:,:4]).float()
  boxes_b = torch.from_numpy(b[:,:4]).float()
  ios = ios_cp(boxes_a, boxes_b)
  ios = ios.data.numpy()
  index = []
  for i in range(ios.shape[0]):
    if (ios[i]>thres).sum()>0:
      continue
    else:
      index.append(i)
  return a[index]
...
def ios_cp(box_a, box_b):
  ...
  area_self = w_a* h_a 
  #union = w_a* h_a + w_b* h_b - (inter[:, :, 0] * inter[:, :, 1])
  return (inter[:, :, 0] * inter[:, :, 1])/area_self

def filter_boxinbox(dets, thres=0.5):
    x1 = dets[:, 0]
    y1 = dets[:, 1]
    x2 = dets[:, 2]
    y2 = dets[:, 3]
...
if __name__=='__main__':
  ...
  results_postprocess()
  json_merge()
  shutil.copy('/home/admin/jupyter/6198_1.json', './tianchi_json/6198.json')
  shutil.copy('/home/admin/jupyter/9379_1.json', './tianchi_json/9379.json')
  recursive_glob()
