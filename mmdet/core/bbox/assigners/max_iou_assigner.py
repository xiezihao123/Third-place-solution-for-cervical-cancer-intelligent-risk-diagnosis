...
class MaxIoUAssigner(BaseAssigner):
    ...
    def assign_IoU(self, gt_bboxes, gt_labels, img_meta, reg_sample=False, num_sample=64, fraction=0.25):
        new_bboxes_list = []
        iou_targets_list = []
        bbox_targets_list = []
        gt_indexes_list = []
        ...
        for i in range(gt_bboxes.shape[0]):
            ...
            iou_targets_multicat = iou_targets.new_zeros(iou_targets.shape[0], 3)
            for k in range(iou_targets.shape[0]):
                iou_targets_multicat[k][gt_label] = iou_targets[k]
            new_bboxes_list.append(new_bbox)
            iou_targets_multicat = iou_targets_multicat*2 - 1
            iou_targets_list.append(iou_targets_multicat)
        ...
        '''
        if iou > 0.5 and iou <=0.6:
            if sample_cnt[0] < num_sample * fraction:
                sample_cnt[0] += 1
                new_bboxes_list.append(new_bbox)
        elif iou > 0.6 and iou <=0.7:
            if sample_cnt[1] < num_sample * fraction:
                sample_cnt[1] += 1
                new_bboxes_list.append(new_bbox)
        elif iou > 0.7 and iou <=0.8:
            if sample_cnt[2] < num_sample * fraction:
                sample_cnt[2] += 1
                new_bboxes_list.append(new_bbox)
        elif iou > 0.8:
            if sample_cnt[3] < num_sample * fraction:
                sample_cnt[3] += 1
                new_bboxes_list.append(new_bbox)

        enough_or_not = (sample_cnt >= num_sample * fraction)
        '''
