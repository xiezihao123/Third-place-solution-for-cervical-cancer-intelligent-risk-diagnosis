...
class RandomErasing(object):
    ...
    def __init__(self, probability=0.5, sl=25, sh=100, r1=0.8, mean=[0.4914*255, 0.4822*255, 0.4465*255]):
    ...
    def __call__(self, results):
        ...
        for attempt in range(100):
            ...
            target_area = random.randint(self.sl, self.sh)
            ...
            if w_ < w and h_ < h:
                x1 = random.randint(0, h - h_)
                y1 = random.randint(0, w - w_)

                img[x1:x1 + h_, y1:y1 + w_, 0] = self.mean[0]
                img[x1:x1 + h_, y1:y1 + w_, 1] = self.mean[1]
                img[x1:x1 + h_, y1:y1 + w_, 2] = self.mean[2]
            results['img']=img
            # return results
...
class Rotate(object):
    ...
    """
    def __init__(self, probablity=0.75):
        self.probablity = probablity

    def bbox_rotate(self, bboxs, img_shape,mat):
        ...
        """
        assert bboxs.shape[-1] % 4 == 0
        h = img_shape[0]
        w = img_shape[1]
        flipped = bboxs.copy()
        # if seed==0:
        flipped[..., 0::4] = w - bboxs[..., 2::4] - 1
        flipped[..., 2::4] = w - bboxs[..., 0::4] - 1
        # else:
        ...
        t_bboxs[:,[0,2]] = np.clip(t_bboxs[:,[0,2]],0,h)
        t_bboxs[:,[1,3]] = np.clip(t_bboxs[:,[1,3]],0,w)
        return t_bboxs
    ...
    def __call__(self, results):
        ...
        # if results['flip']:
            ...
            mat = cv2.getRotationMatrix2D(center=center,angle=angle,scale=1.0)
            for key in results.get('bbox_fields', []):
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(rotate_angel={})'.format(
            self.angel)



@PIPELINES.register_module
class RandomFlip__(object):
    ...
    def bbox_flip(self, bboxes, img_shape,seed=0):
        """Flip bboxes horizontally.
...
@PIPELINES.register_module
class RandomFlip_Vertical(object):
    """Flip the image & bbox & mask.
...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('bbox_fields', []):
            ...
            bboxes[:, 1::2] = np.clip(bboxes[:, 1::2], 0, img_shape[0] - 1)
            results[key] = bboxes
        print('bbox_fields:', results['bbox_fields'])
        ...
        if 'gt_bboxes' in results:
            ...
            if not np.any(valid_inds):
                return None
            print('gt_bboxes shape:', results['gt_bboxes'].shape)
            print('gt_bboxes:', 'gt_bboxes')
            results['gt_bboxes'] = gt_bboxes[valid_inds, :]
            print('gt_bboxes:', 'gt_bboxes')
            if 'gt_labels' in results:
                print('gt_label shape:',results['gt_labels'].shape)
                print('gt_label:', results['gt_labels'])
