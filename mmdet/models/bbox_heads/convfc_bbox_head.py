...
class IoU_head(nn.Module):
    ...
    def loss(self,
        ...
        if IoU_pred is not None:
            ...
            if IoU_rank:
                indexs = torch.unique(gt_indexes)
                for i in range(indexs.shape[0]):
                    local_index = (gt_indexes == indexs[i]).nonzero().squeeze(dim=1)
                    local_num = local_index.shape[0]
                    _, rank = torch.sort(IoU_targets[local_index], descending=True)
                    max_index = local_index[rank[0]]

                    gt_offset = IoU_targets[max_index] - IoU_targets[local_index]
                    predict_offset = IoU_pred[max_index] - IoU_pred[local_index]
                    criteria = gt_offset * (predict_offset + 0.05)
                    enlarge_index = (criteria <= 0).nonzero().squeeze(dim=1)
                    IoU_weights[local_index[enlarge_index]] = 30
    ...
    def get_det_bboxes(self,
                       ...
                       scale_factor,
                       rescale=False,
                       cfg=None, gt_bboxes=None, gt_labels=None):
        ...
        if cfg is None:
            ...
            #ious = bbox_overlaps(bboxes, gt_bboxes[0][0])
            ious = bbox_overlaps(bboxes, rois[:, 1:], is_aligned=True)
            max_iou, index = ious.max(dim=1)
            det_bboxes, det_labels = multiclass_nms(bboxes, scores,
                                                    cfg.score_thr, cfg.nms,
                                                    cfg.max_per_img, score_factors=IoU_pred)

            return det_bboxes, det_labels, max_iou, bboxes
    ...
    def get_final_det_bboxes(self,
        ...
        if cfg is None:
            ...
            #return det_bboxes, det_labels, max_iou, bboxes
            return det_bboxes, det_labels, bboxes
