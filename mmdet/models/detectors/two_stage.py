...
class TwoStageDetector(BaseDetector, RPNTestMixin, BBoxTestMixin,
    ...
    def init_weights(self, pretrained=None):
        ...
        if self.with_IoU:
            self.IoU_head.init_weights()
            if not self.share_roi_extractor:
                self.IoU_roi_extractor.init_weights()
        if self.with_reg:
    ...
    def forward_train(self,
        ...
        if self.with_bbox or self.with_mask:
            ...
            for i in range(num_imgs):
                ...
                if self.with_IoU or self.with_reg:
                    IoU_bbox_sample, IoU_targets, gt_indexes, bbox_targets = bbox_assigner.assign_IoU(gt_bboxes[i], gt_labels[i], img_meta[i], reg_sample=True)
        ...
        if self.with_bbox:
            ...
            for i in range(len(sampling_results)):
                index_gt = sampling_results[i].pos_assigned_gt_inds+1000*i
                index_gt_list.append(index_gt)

            indexs_gt = torch.cat(index_gt_list, 0)
        ...
        if self.with_IoU:
            ...
            #     loss_reg = self.reg_head.loss(reg_pred, bbox_targets)
