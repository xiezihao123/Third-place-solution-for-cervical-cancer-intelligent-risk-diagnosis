    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if self.with_bbox or self.with_mask:
                ...
                for j in range(num_imgs):
                    ...
                    if self.with_IoU or self.with_reg:
                        IoU_bbox_sample, IoU_targets, gt_indexes, bbox_targets = bbox_assigner.assign_IoU(gt_bboxes[j], gt_labels[j], img_meta[j], reg_sample=True)
            ...
            if self.with_IoU:
                ...
                IoU_pred = IoU_head(IoU_bbox_feats)
                #     loss_IoU = self.IoU_head.loss(bbox_pred, bbox_targets, IoU_pred, IoU_targets, gt_indexes)
                #     losses.update(loss_IoU)
                loss_IoU = IoU_head.loss(IoU_pred, IoU_targets, gt_indexes)
                losses.update(loss_IoU)
            # refine bboxes
